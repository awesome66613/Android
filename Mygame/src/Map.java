...
public class Map extends JPanel {
    ...
    void modeTwoPlayer(int y, int x) {
        ...
        repaint();
        if (checkWin(cymbal)) {
            gameOverWindow.setMessage(stepPlayer2 ? "Первый игрок ВЫИГРАЛ!!" : "Второй игрок ВЫИГРАЛ!!",this);
            return;
        }
        if (isFuelFull()) {
    ...
    void modePlayAI(int y, int x) {
        ...
        repaint();
        if (checkWin(PLAYER_DOT)) {
            gameOverWindow.setMessage("Ты ВЫИГРАЛ!",this);
            return;
        }
        if (isFuelFull()) {
            gameOverWindow.setMessage("НИЧЬЯ!",this);
        ...
        repaint();
        if (checkWin(AI_DOT)) {
            gameOverWindow.setMessage("Машины ПОБЕДИЛИ!",this);
            return;
        }
        if (isFuelFull()) {
            gameOverWindow.setMessage("НИЧЬЯ!",this);
